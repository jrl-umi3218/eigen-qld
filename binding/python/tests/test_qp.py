...
class TestQLD(unittest.TestCase):
  def test(self):
    qp1 = QP1()
    ...
    self.assertAlmostEqual((qld.result() - qp1.X).norm(), 0, 6)

class TestQLDSize(unittest.TestCase):
  def test(self):
    qp1 = QP1()

    qld = eigen_qld.QLD(qp1.nrvar, qp1.nreq + 10, qp1.nrineq + 22)
...
class TestIneqWithXBounds(unittest.TestCase):
  def test(self):
    ...
    inf = float('inf')
    for i in range(qp1.nrvar):
      qp1.XL[i] = -inf
      qp1.XU[i] = inf

    nrineq = Aineq.rows()
    qld = eigen_qld.QLD(qp1.nrvar, qp1.nreq, nrineq)

    qld.solve(qp1.Q, qp1.C, qp1.Aeq, qp1.Beq, Aineq, Bineq, qp1.XL, qp1.XU)
